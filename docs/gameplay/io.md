...
| Delay | Seconds to wait before calling the input, 0 calls it at once |
| Times | How often the connection may fire. -1 means no limit, 1 makes a one shot event |
...
| `arrived(corner)`, `spawned(node)`, `reached(train)` | Replaced by the node the signal carries |
| `opened`, `timer`, `step_N`, `on_true`, `broken`, `switched`, `map_spawn` | Dropped, these carry no node |
