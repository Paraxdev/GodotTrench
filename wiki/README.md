...
Every element on the shipped pages is one the editor itself creates, so a page edited by hand should stay within
what the editor writes:

- A document starts a new paper at every card that opens with a `#` or `##` heading (the Heading 1 and Heading 2 blocks).
- Definition lists are a term line followed by a `: definition` line, and consecutive pairs form one list
  (the Definition list block).
- A text card with `"variant": "entry"` is an entry panel, a raised slab that opens with a `###` heading
  (the Entry panel block, or "Show as an entry panel" in the block menu).
- Tables are Markdown tables inside text cards (the Table block). The older `table` card type still renders but is
  no longer offered.

Canvas cards keep `x`, `y`, `w`, `h` and `z`, and a text card can set `variant` to `note`, `plain` or `entry`.
Connectors are an optional `edges` list next to the cards, each `{ "id", "from", "to", "label" }`, where `from` and
`to` are card ids and `label` is optional.
