...
pub fn tool_definitions() -> Vec<Value> {
    ...
    vec![
        ...
        json!({
            "name": "walkability",
            "description": "Where an agent can walk. The Godot editor connected over the live link bakes a navigation mesh of the scene that builds this map, from the collision the game has (clip brushes block, triggers, doors and platforms do not), and the 3D and 2D views show it: the biggest connected area in bluish green, every island cut off from it in vermillion. Returns the walkable area in square meters, the island count, the main area's bounds, and the area, center and bounds (map units) of each cut off island, largest first, so blocked doorways, steps higher than max_climb and gaps narrower than twice the radius can be found without a screenshot. Wall, roof and furniture tops show up as small islands too. Needs the map saved in the open project and a scene open in Godot whose FuncGodotMap builds it. Without live mode Godot first builds the map as shown here, like build_in_godot. The agent sizes are kept for the next call and for the overlay, which bakes again after edits until show: false or run_action toggle_walkable hides it. Cannot run inside scripts.",
            "inputSchema": { "type": "object", "properties": {
                "radius": { "type": "number", "description": "Agent radius in meters, default 0.3 or the last one used" },
                "height": { "type": "number", "description": "Agent height in meters, default 1.8" },
                "max_climb": { "type": "number", "description": "Highest step the agent climbs in meters, default 0.3" },
                "max_slope": { "type": "number", "description": "Steepest walkable slope in degrees, default 45" },
                "show": { "type": "boolean", "description": "Show the overlay in the views, default true" }
            } }
        }),
        json!({
            "name": "simulate_input",
        ...
        json!({
            "name": "hierarchy",
            "description": "Layers and groups. New objects go into the innermost open group, else the current layer. ops: add_layer {name, default Layer N} (becomes current), add_group {name, parent, open (default true)}, open_group {id} (a group), close_group {all}, set_current_layer {id}, reparent {ids, parent} (into a layer or group, brushes and meshes also into a brush entity), rename {id, name} (layers, groups and scatter sets take it as their name and need one, a scatter set's own layer is renamed with it, other objects get a name shown in place of brush12 and the like and an empty name brings the default back, entities keep their targetname), set_flags {ids, hidden, locked, omit_from_export}. Locked layers and groups accept no new objects.",
            "inputSchema": { "type": "object", "properties": {
