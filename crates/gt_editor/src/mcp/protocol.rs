...
pub fn tool_definitions() -> Vec<Value> {
    ...
    vec![
        ...
        json!({
            "name": "hierarchy",
            "description": "Layers and groups. New objects go into the innermost open group, else the current layer. ops: add_layer {name, default Layer N} (becomes current), add_group {name, parent, open (default true)}, open_group {id} (a group), close_group {all}, set_current_layer {id}, reparent {ids, parent} (into a layer or group, brushes and meshes also into a brush entity), rename {id, name} (layers, groups and scatter sets take it as their name, other objects get a name shown in place of brush12 and the like, entities keep their targetname, an empty name brings the default back), set_flags {ids, hidden, locked, omit_from_export}. Locked layers and groups accept no new objects.",
            "inputSchema": { "type": "object", "properties": {
